...
<div class="filament-echarts-header">
    @if ($heading || $subheading || $filters || $filterFormAccessible)
        <div class="sm:flex justify-between gap-4 py-2 relative">

            <div>
                @if ($heading)
                    <div class="filament-echarts-heading text-base font-semibold leading-6">
                        {!! $heading !!}
                    </div>
                @endif

                @if ($subheading)
                    <div class="filament-echarts-subheading text-sm text-gray-600 dark:text-gray-300">
                        {!! $subheading !!}
                    </div>
                @endif
            </div>
